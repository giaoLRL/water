...
| **液位** | 待定（`config.LEVEL_PATH`） | ⏳ **传感器即将加装，尚未接入 → 当前为模拟值** |
| 水温 | `GET /api/temperature` | ❌ 404，固件无此通道 |
| 压力 | — | ❌ `/api/data` 无压力字段 |
| 加热模块 | `GET /api/heater/*` | ❌ 404，固件无此通道 |

> ⚠️ 现场固件与早期接口清单不一致：清单中的 `temperature` / `tempOk` / `heater` 实测均不存在。
>
> 因此本系统为 **纯真实模式 + 按能力裁剪**：温度、压力、加热相关面板、告警与恒温闭环(PID)
> 明确标注「设备不支持」并停用，**不生成任何模拟数据**。设备离线时字段返回 `null`，
> 前端提示离线，绝不用 0 或模拟值冒充真实读数。
>
> 能力开关集中在 `backend/config.py` 的 `DEVICE_FEATURES`；若后续烧录了带温度/压力/加热的
> 固件，把对应项改成 `True` 即可自动启用相应采集、告警、接口与前端面板。
...
- **双水槽实时水位**：储水槽与加热槽各一个罐体，水面波浪、气泡上升、外侧刻度、罐内大字水位百分比。
  ⚠️ 两路液位传感器**尚未接入**，当前水位为**模拟值**：按「水量守恒」模拟双水槽循环——
  水泵运行时把水从储水槽送到加热槽（储水槽保留最低水位防抽干），停机后两槽缓慢回平。
  界面与接口均明确标注 `source: "sim"` / 「模拟」徽标，不代表真实水槽状态。
  设备掉线时模拟水位仍会继续演化，不会冻结。
...
# 2) 模拟液位单元测试（不访问网络/数据库，可独立运行）
python backend\tests\test_level_sim.py
...
- **双水槽液位传感器接入后如何切换为实测值**：把 `backend/config.py` 的
  `DEVICE_FEATURES["level_storage"]` / `["level_heater"]` 改为 `True`，并按固件实际接口填写
  `LEVEL_PATH_STORAGE` / `LEVEL_PATH_HEATER`（默认 `/api/level/storage`、`/api/level/heater`）
  与 `TANK_HEIGHT_CM_STORAGE` / `TANK_HEIGHT_CM_HEATER`（水槽高度，cm）。
  采集端支持返回百分比（`unit:"%"`）或厘米（`unit:"cm"`/`"mm"`，自动按高度换算）。
  改完重启后端即可，界面会自动由「模拟」切换为「实测」；两槽可分别接入，互不影响。
