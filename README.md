...
- **报警联动（可编辑的告警动作，个体化）**：在系统配置页配置「哪个传感器的阈值 → 触发哪个执行器」：
  触发源可选本机传感器（水流量/双温/水压/光照）或自定义接口传感器（URL+取值路径，后端自动轮询），
  每条规则自带阈值与方向；动作显式指向执行器个体——本机水泵/本机加热/取消定量，或执行器档案中的
  外部装置（如水阀：名称+开/关 GET URL）。越限沿触发，回正常沿可选执行恢复动作；指令经后端代理
  下发（白名单主机）并写入操作日志（来源标注为系统自动），全程可审计。
- **自定义传感器通道（全链路）**：在系统配置页声明外部传感器接口（URL+取值路径+轮询周期+单位），
  后端采集循环自动轮询入库——历史曲线页出现对应标签、统计页出现指标卡、联动规则可直接引用通道、
  越限/恢复写入告警记录；实时监控加一张「自定义接口」卡（取值路径填 `custom.通道id`）即可看实时值。
...
| `/api/water/custom/channels` | GET/POST | 自定义传感器通道声明（后端轮询入库；写 cfg_system） |
| `/api/water/custom/history`、`/api/water/custom/stats` | GET | 自定义通道历史曲线与统计指标（按通道+时间范围） |
